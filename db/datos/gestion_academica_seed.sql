...
INSERT INTO cursos (id, docente_titular_id, periodo_id, nombre, descripcion, cupo, estado) VALUES
    ('cur-101', 'doc-001', 'per-2026-c1', 'Bases de Datos Relacionales', 'Modelado conceptual, logico y fisico; normalizacion y SQL.', 40, 'en_curso'),
    ('cur-102', 'doc-003', 'per-2026-c1', 'Diseno de Soluciones de Datos', 'Arquitecturas de datos, indices, NoSQL y proyecto integrador.', 35, 'en_curso')
ON CONFLICT (id) DO NOTHING;

-- Inscripciones (N:M estudiante-curso). est-001 cursa dos materias (muestra la cardinalidad).
INSERT INTO inscripciones (id, estudiante_id, curso_id, fecha_inscripcion, estado) VALUES
    ...
    ('ins-004', 'est-004', 'cur-102', '2026-02-27', 'activa'),
    ('ins-005', 'est-001', 'cur-102', '2026-03-02', 'activa')
ON CONFLICT (id) DO NOTHING;
